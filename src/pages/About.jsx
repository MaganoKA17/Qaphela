   ...
   <h2 style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {[
            {
              icon: '🔒',
              title: 'Message content is never stored',
              desc: 'Your message is analyzed and immediately discarded. Only the risk score, signals, and AI explanation are retained.',
            },
            {
              icon: '👤',
              title: 'No user identification',
              desc: 'Qaphela requires no account, login, or personal information. Nothing about you is collected or stored.',
            },
            {
              icon: '📊',
              title: 'Anonymised results only',
              desc: 'The community scam feed contains only analytical metadata — risk scores, signal types, and AI explanations. No message content, phone numbers, or identifying information.',
            },
            {
              icon: '🇿🇦',
              title: 'POPIA compliant by design',
              desc: "Qaphela applies data minimisation principles in line with South Africa's Protection of Personal Information Act. Only the minimum data required to power the community feed is retained.",
            },
          ].map((item, i) => (
            <div key={i} style={{
              display: 'flex', gap: '14px', alignItems: 'flex-start',
